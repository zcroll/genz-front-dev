<template>
  <div class="mb-8 border-b dark:border-gray-700">
    <div v-if="isLoading" class="flex justify-center py-4">
      <div class="animate-spin rounded-full h-6 w-6 border-b-2" :style="{ borderColor: `var(--${themeColorName}-500)` }"></div>
    </div>
...
const emit = defineEmits(['navigation-loaded']);
...
const availableNavItems = computed(() => {
  return navigationItems.value.filter(item => item.available);
});

// Function to convert API URI to router path
const getRouteFromUri = (uri: string): { name: string, params: { slug: string } } => {
